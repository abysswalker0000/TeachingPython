...
df = pd.DataFrame(data)
max_speed = df.loc[df["Скорость"].idxmax(), ["Имя", "Скорость"]]
min_weight = df.loc[df["Вес"].idxmin(), ["Имя", "Вес"]]
print(f"Спортсмен с макс скоростью - {max_speed["Имя"]}({max_speed["Скорость"]})")
print(f"Спортсмен с минимальным весом - {min_weight["Имя"]}({min_weight["Вес"]})")